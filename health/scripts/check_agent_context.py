...
def main() -> int:
    ...
    parser.add_argument("--include-global", action="store_true", help="Explicitly include global Agent configuration")
    parser.add_argument(
    ...
    _AUDIT_ROOT = root
    _AUDIT_HOME = home if args.include_global else None
    ...
    if not args.include_global:
        print("GLOBAL_SCOPE: excluded (global absence is not a finding)")
    ...
    if not args.include_global:
        # Do not enumerate global directories merely to report them excluded.
        print(json.dumps({
            "scope": "project", "global": "excluded", "mode": mode,
            "instruction_files": [rel(path, root) for path in instruction_files],
            "instruction_findings": instruction_findings,
            "claude_aliases_agents": claude_aliases_agents,
            "claude_delegates_to_agents": claude_delegates,
