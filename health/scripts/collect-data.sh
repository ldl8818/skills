...
for ((index=0; index<${#arguments[@]}; index++)); do
  case "${arguments[index]}" in
    --root) TARGET_ARGUMENT="${arguments[index+1]:-}" ;;
    --root=*) TARGET_ARGUMENT="${arguments[index]#--root=}" ;;
  esac
done
HEALTH_TARGET=$(cd "$TARGET_ARGUMENT" 2>/dev/null && pwd -P) || exit 2
...
exec "$PYTHON_BIN" -I "$SCRIPT_DIR/collect_data.py" "$@"
