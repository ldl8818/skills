...
不再需要日常查看时，可以将旧 `corrections.md` 移入自己的归档目录。3.1.1起，已有标记的记忆目录不会在初始化时重建该文件，体检也不要求它存在；自动候选和新版审批仍分别写入原候选箱与账本。归档只供人工追溯。
...
拒绝提示中的 `[authority-guard]` 表示命中保护检测，不等于已写入记忆。若原命令是只读检查，先排查误拦；memory.md 与 corrections.md 的授权修改和移动可由 Agent 执行；审批账本及变更审核命令保留额外保护。字符串前缀判断、负数下标，以及搜索后接只读审核列表的常见命令已支持。
