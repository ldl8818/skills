...
分层判据：`scripts/opencli-health.sh` 只检查已绑定的 `ego-lite` Profile，并区分休眠后成功启动、daemon 启动失败、扩展断连、Profile 未绑定／断连／错配；门禁 ready 后取数为空是 L4；平台报 `AUTH_REQUIRED` 是 L2。门禁只读 daemon `/status`，不发平台请求。登录态只在需要时用 `node scripts/opencli-run.mjs auth status --site <site> --timeout 8 -f json` 有界检查。别为了分层去跑 `opencli doctor`——它会留下一个关不掉的空白窗口，见 `opencli-windows.md` 。
